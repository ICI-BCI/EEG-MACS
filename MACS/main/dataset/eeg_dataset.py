...
def get_dataset(args, transform_train1, transform_train2,transform_test,k):
    ...
    xtrain_norm = (xtrain - mean_per_channel) / std_per_channel
    xtrain=torch.Tensor(xtrain_norm).unsqueeze(1).float()

    xtest_norm = (xtest - mean_per_channel) / std_per_channel
    xtest=torch.Tensor(xtest_norm).unsqueeze(1).float()
